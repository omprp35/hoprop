...
if [[ -z "$DESKTOP_PASSWORD" ]]; then
  echo "ERROR: DESKTOP_PASSWORD is required." >&2
  echo "Add DESKTOP_PASSWORD in Railway Variables and redeploy." >&2
  exit 1
fi
...
cleanup() {
  kill "${NGINX_PID:-}" "${NODE_PID:-}" "${WS_PID:-}" "${VNC_PID:-}" "${WM_PID:-}" "${XVFB_PID:-}" 2>/dev/null || true
}
...
echo "Starting Telegram/live-desktop app on internal port $APP_PORT..."
node src/server.js &
NODE_PID=$!
...
echo "Starting nginx on Railway port $PUBLIC_PORT..."
